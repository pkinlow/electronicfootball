...
class FootballPlayer extends Component {
  render() {
    const defenseOrOffense = this.props.val === DEFENSE ? "defense" : "offense";
    return (
      <React.Fragment>
        <span className={"subject " + defenseOrOffense }>{ subsitutes[this.props.val] || this.props.val }</span>
      </React.Fragment>
    );
  }
}
...
class Yard extends Component {
  render() {
    const { position, subject } = this.props;
    const fieldPosition = position + "field";
    return (
      <div className={"grassfield " + fieldPosition}>
        <FootballPlayer val={subject} />
      </div>
    );
  }
}
...
class Field extends Component {
  setYards(fieldData, subjectVal) {
    ...
    const yards = fieldData
      .map((yardlines, pos) => {
        return yardlines
          .map((val) => {
            return {
              subject: subjectVal(val),
              position: positions[pos]
            };
          });
      });
    
    return yards.flat().map((yardData, index) => {
      return <Yard key={index} {...yardData} />;
    });
  }
  ...
  render() {
    return (
      <div>
        ...
        <a className="gamebutton" onClick={(e) => {e.preventDefault(); this.moveLeft(this.state.qb);}}><span>Left</span></a>
        <a className="gamebutton" onClick={(e) => {e.preventDefault(); this.moveUp(this.state.qb);}}><span>Up</span></a>
        <a className="gamebutton" onClick={(e) => {e.preventDefault(); this.moveDown(this.state.qb);}}><span>Down</span></a>
        <a className="gamebutton" onClick={(e) => {e.preventDefault(); this.moveRight(this.state.qb);}}><span>Right</span></a>
        <br />
        {/* <button className="gamebutton" onClick={(e) => {e.preventDefault; this.defenseMove();}}><span>Move Defense</span></button> */}
      </div>
    );
  }
}
